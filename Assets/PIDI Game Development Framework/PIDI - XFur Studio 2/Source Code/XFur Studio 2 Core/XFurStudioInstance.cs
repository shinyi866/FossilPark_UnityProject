...
namespace XFurStudio2 {
    ...
    public class XFurStudioInstance : MonoBehaviour {
        ...
        /// <summary> Whether this XFur Instance should be adjusted to work on Mobile devices</summary>
        //public bool MobileMode = false;

        //public bool ForceBakedData = false;
        ...
        /// <summary> The version of the XFur Mesh Patcher expected by this instance</summary>
        //private string patcherVersion = "2.0";

        /// <summary>The current version of this asset. Used internally for the auto-update routines and UI displays</summary>
        private string AssetVersion = "v2.1.3";
        ...
        public XFurStudioDatabase FurDatabase { get { return furDatabase; }
#if UNITY_EDITOR
            ...
            for ( int i = 0; i < mainRenderer.materials.Length; ++i ) {
                if ( mainRenderer.isFurMaterial[i] ) {
                    ...
                    if ( vfxModule.enabled && Application.isPlaying ) {
                        ...
                        furBlocks[i].SetTexture( "_XFurVFXMask", Texture2D.blackTexture );
                    }
                    ...
                    if ( physicsModule.enabled && Application.isPlaying ) {
                        ...
                        furBlocks[i].SetTexture( "_XFurPhysics", Texture2D.blackTexture );
                    }
                    ...
                    if ( RenderingMode == XFurRenderingMode.XFShells ) {
                        ...
                        furBlocks[i].SetTexture( "_XFurSkinMap", furDataProfiles[i].SkinColorMap ? furDataProfiles[i].SkinColorMap : Texture2D.whiteTexture );
                        if ( furDataProfiles[i].SkinNormalMap )
                            furBlocks[i].SetTexture( "_XFurSkinNormal", furDataProfiles[i].SkinNormalMap );
                    }


                    if ( FurDatabase.RenderingMode == XFurStudioDatabase.XFurRenderingMode.HighDefinition ) {
                        furBlocks[i].SetMatrix( "_XFurHDWorld2LocalMatrix", MainRenderer.renderer.transform.worldToLocalMatrix );
                        furBlocks[i].SetMatrix( "_XFurHDLocal2WorldMatrix", MainRenderer.renderer.transform.localToWorldMatrix );
                    }
                    ...
                    if ( !Application.isPlaying || forceUpdate || ( AutoUpdateMaterials && Time.realtimeSinceStartup > timedUpdate ) ) {
                        ...
                        furBlocks[i].SetColor( "_XFurRimColor", furDataProfiles[i].FurRim );
        ...
        private void RenderFur() {
            ...
            if ( RenderingMode == XFurRenderingMode.XFShells ) { 
                ...
                if ( skipRenderFrame || !CurrentFurRenderer.renderer.isVisible || !CurrentFurRenderer.renderer.enabled ) {
