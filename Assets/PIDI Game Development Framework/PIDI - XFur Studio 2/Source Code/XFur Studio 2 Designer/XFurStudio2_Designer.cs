namespace XFurStudio2 {
    ...
    public class XFurStudio2_Designer : MonoBehaviour {
        ...
        private void OnGUI() {
            ...
            pidiSkin2.label.fontSize = Mathf.RoundToInt( 14 * scale );
            pidiSkin2.button.fontSize = Mathf.RoundToInt( 12 * scale );
            pidiSkin2.customStyles[0].fontSize = Mathf.RoundToInt( 14 * scale );
            ...
            GUI.color = new Color( 0.75f, 0.75f, 0.75f );
            GUILayout.BeginArea( new Rect( 0, 40 * scale, 220 * scale, 530 * scale ), pidiSkin2.customStyles[0] );
            {
                GUI.color = Color.white;
                GUILayout.Box( "Brush Settings", pidiSkin2.customStyles[0], GUILayout.Height( 50 * scale ) );

                GUI.DrawTexture( new Rect( 40 * scale, 70 * scale, 140 * scale, 140 * scale ), brushPreview );
                ...
                var designerDropRect = new Rect( 28 * scale, 290 * scale, 160 * scale, 150 * scale );
                var furPropertyDropRect = new Rect( 28 * scale, 360 * scale, 160 * scale, 120 * scale );
               ...
               case 4:
                ...
                if ( designerModeDrop || furPropertyDrop ) {
                    SliderField( "Size", brushSize, 0, 2 );
                    SliderField( "Hardness", brushHardness, 0, 1 );
                    if ( designerMode > 0 ) {
                        SliderField( "Opacity", brushOpacity, 0, 1 );
                    }
                }
                else {
                    brushSize = SliderField( "Size", brushSize, 0, 2 );
                    brushHardness = SliderField( "Hardness", brushHardness, 0, 1 );
                    if ( designerMode > 0 ) {
                        brushOpacity = SliderField( "Opacity", brushOpacity, 0, 1 );
                    }
                }
            ...
            GUI.color = new Color( 0.75f, 0.75f, 0.75f );
            GUILayout.BeginArea( new Rect( 0, 570 * scale, 220 * scale, 230 * scale ), pidiSkin2.customStyles[0] );
            {
                GUI.color = Color.white;
                ...
                pidiSkin2.customStyles[0].fontSize = Mathf.RoundToInt( 12 * scale );
                GUILayout.BeginHorizontal(); GUILayout.FlexibleSpace();
            ...
            GUI.color = new Color( 0.75f, 0.75f, 0.75f );
            GUILayout.BeginArea( new Rect( 0, 800 * scale, 220 * scale, Screen.height - 800 * scale ), pidiSkin2.customStyles[0] );
            {
                GUI.color = Color.white;
            ...
            if ( showColorPickerL ) {
                GUI.color = new Color( 0.75f, 0.75f, 0.75f );
                GUILayout.BeginArea( new Rect( 220 * scale, 200 * scale, 200 * scale, 320 * scale ), pidiSkin2.customStyles[0] );
                GUI.color = Color.white;
                ...
                var colorRect = new Rect( 252 * scale, 232 * scale, 138 * scale, 138 * scale );
#if ENABLE_INPUT_SYSTEM
            ...
            if ( Input.GetMouseButton( 0 ) || targetColor != brushColor ) {
                ...
                if ( Event.current != null && Event.current.type == EventType.MouseDown ) {
#if ENABLE_INPUT_SYSTEM
                    if ( !new Rect( 220 * scale, 200 * scale, 200 * scale, 320 * scale ).Contains( new Vector2( Mouse.current.position.ReadValue().x, Screen.height - Mouse.current.position.ReadValue().y ) ) )
#else
                    if ( !new Rect( 220 * scale, 200 * scale, 200 * scale, 320 * scale ).Contains( new Vector2( Input.mousePosition.x, Screen.height - Input.mousePosition.y ) ) )
#endif
            ...
            GUI.color = new Color( 0.75f, 0.75f, 0.75f );
            GUILayout.BeginArea( new Rect( Screen.width - 230 * scale, 40 * scale, 230 * scale, 500 * scale ), pidiSkin2.customStyles[0] );
            {
                GUI.color = Color.white;
            ...
            GUI.color = new Color( 0.75f, 0.75f, 0.75f );
            GUILayout.BeginArea( new Rect( Screen.width - 230 * scale, 540 * scale, 230 * scale, 360 * scale ), pidiSkin2.customStyles[0] );
            {
                GUI.color = Color.white;
            ...
            GUI.color = new Color( 0.75f, 0.75f, 0.75f );
            GUILayout.BeginArea( new Rect( Screen.width - 230 * scale, 900 * scale, 230 * scale, Screen.height - 900 * scale ), pidiSkin2.customStyles[0] );
            {
                GUI.color = Color.white;
            ...
            if ( showColorPickerR ) {
                GUI.color = new Color( 0.75f, 0.75f, 0.75f );
                GUILayout.BeginArea( new Rect( Screen.width - 430 * scale, 200 * scale, 200 * scale, 320 * scale ), pidiSkin2.customStyles[0] );
                GUI.color = Color.white;
                GUI.DrawTexture( new Rect( 30 * scale, 30 * scale, 140 * scale, 140 * scale ), colorPicker );
                ...
                var colorRect = new Rect( Screen.width - 398 * scale, 232 * scale, 138 * scale, 138 * scale );
#if ENABLE_INPUT_SYSTEM
        ...
        public void Update() {
            ...
            if ( !Keyboard.current.shiftKey.isPressed && !Keyboard.current.ctrlKey.isPressed ) {
                if ( Physics.Raycast( cam.ScreenPointToRay( Mouse.current.position.ReadValue() ), out RaycastHit hit, meshBounds.size.magnitude * 10 ) ) {
                    canPaint = Mouse.current.position.ReadValue().x > 220 * scale && Mouse.current.position.ReadValue().x < Screen.width - 220 * scale && !showColorPickerL && !showColorPickerR;

#else
            if ( !Input.GetKey( KeyCode.LeftShift ) && !Input.GetKey( KeyCode.LeftControl ) ) {
                if ( Physics.Raycast( cam.ScreenPointToRay( Input.mousePosition ), out RaycastHit hit, meshBounds.size.magnitude * 10 ) ) {
                    canPaint = Input.mousePosition.x > 220 * scale && Input.mousePosition.x < Screen.width - 220 * scale && !showColorPickerL && !showColorPickerR;
            ...
            if ( Keyboard.current.shiftKey.isPressed ) {
                ...
                if ( Mouse.current.leftButton.isPressed ) {
                    if ( Mathf.Abs( Mouse.current.delta.ReadValue().x * Time.deltaTime ) > Mathf.Abs( Mouse.current.delta.ReadValue().y * Time.deltaTime ) ) {
                        brushSize = Mathf.Clamp( brushSize + Mouse.current.delta.ReadValue().x * Time.deltaTime * 0.05f, 0.005f, 2f );
                    }
                    else {
            ...
            if ( Input.GetKey( KeyCode.LeftShift ) ) {
                ...
                if ( Input.GetMouseButton( 0 ) ) {
                    if ( Mathf.Abs( Input.GetAxis( "Mouse X" ) ) > Mathf.Abs( Input.GetAxis( "Mouse Y" ) ) ) {
                        brushSize = Mathf.Clamp( brushSize + Input.GetAxis( "Mouse X" ) * 0.05f, 0.005f, 2f );
                    }
                    else {
            ...
            if ( !Keyboard.current.shiftKey.isPressed && Mouse.current.middleButton.isPressed ) {
                orbitX += Mouse.current.delta.ReadValue().x * Time.deltaTime * 20f * distance;
            ...
            if ( !Input.GetKey( KeyCode.LeftShift ) && Input.GetMouseButton( 2 ) ) {
                orbitX += Input.GetAxis( "Mouse X" ) * 350f * distance * Time.deltaTime;
            ...
            for ( int i = 0; i < TargetInstances.Count; i++ ) {
                ...
                if ( !TargetInstances[i].targetXFur.CurrentFurRenderer.renderer.GetComponent<MeshCollider>() ) {
                    TargetInstances[i].targetXFur.CurrentFurRenderer.renderer.gameObject.AddComponent<MeshCollider>();
                }

                TargetInstances[i].targetXFur.CurrentFurRenderer.renderer.GetComponent<MeshCollider>().sharedMesh = targetMesh;
...
#region PIDI 2020 EDITOR
        ...
        public void HelpBox( string message, MessageType messageType ) {
            ...
            tStyle.fontSize = Mathf.RoundToInt( 12 * scale );
        ...
        /// <returns></returns>
        public float SliderField( string label, float currentValue, float minSlider = 0.0f, float maxSlider = 1.0f, float width = 80, bool showNumber = false ) {
